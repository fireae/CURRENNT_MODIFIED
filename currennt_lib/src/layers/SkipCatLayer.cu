...
namespace layers{
	...
	// Check
	if (m_preSkipDim.size() != m_preLayers.size() * 2)
	    throw std::runtime_error("Error: one in preSkiplayer, two in preSkipLayerDim");

	m_preSkipDimAccu.resize(m_preLayers.size(), 0);
	int tmpDim = 0;
	for (int cnt =0 ; cnt < m_preLayers.size(); cnt ++){
	    if (m_preSkipDim[2*cnt + 1] > m_preLayers[cnt]->size() || m_preSkipDim[2*cnt] < 0 ||
		m_preSkipDim[2*cnt + 1] < m_preSkipDim[2*cnt]){
		printf("Error preSkipLayerDim for %s\n", m_preLayers[cnt]->name().c_str());
	...
	{{
		...
		thrust::for_each(
		   thrust::make_zip_iterator(
			thrust::make_tuple(
				this->outputs().begin()           + effTimeS * fn.copyDim, 
				thrust::counting_iterator<int>(0) + effTimeS * fn.copyDim)),
		   thrust::make_zip_iterator(
			thrust::make_tuple(
				this->outputs().begin()           + effTimeE * fn.copyDim, 
